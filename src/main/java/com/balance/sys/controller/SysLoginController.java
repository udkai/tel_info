...
public class SysLoginController {
    ...
    public void checkLogin(HttpServletRequest request, HttpServletResponse response, String username, String password, String verifyCode) {
        ...
        if (sysUser == null || !sysUserService.checkPass(sysUser, password)) {
            ...
            if (sysUser.getStatus() == 2 || sysUser.getStatus() == 9) {
                ...
                request.getSession().setMaxInactiveInterval(30*60);// 设置过期时间30分钟 单位是秒
                // 插入登录日志
    ...
    public ModelAndView toUpdatePassword(HttpServletRequest request, HttpServletResponse response, SysUser sysUser) {
        ModelAndView mv = new ModelAndView();
        mv.setViewName("/user/updatePassword");
        mv.addObject("backUrl", "/customer/mobile/index.htm");
        return mv;
    }
